...
│   ├── notification-card/
│   └── pdf-viewer/
├── hooks/                # shared React hooks (`use*`) — not query helpers
├── queries/              # query keys, types, ensureQueryData helpers
...
import { getNotificationsFromQueryClient } from '@/queries/notifications'
...
src/
├── queries/
│   └── notifications.ts         # NOTIFICATIONS_KEY, getNotificationsFromQueryClient()
├── lib/
...
| Query modules | `<resource>.ts` in `queries/` | `queries/social-providers.ts` |
| React hooks | `use-<name>.ts` in `hooks/` | `use-media-query.ts` |
...
useEffect(function fetchNotifications() { fetch('/api/...') }, [])
