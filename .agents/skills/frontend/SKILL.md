---
name: frontend
description: Frontend conventions for the Vite client app — component colocation, page structure, PageContainer, Breadcrumb, and PageHeader layout, shared components, query modules in src/queries/, server data fetching, and named useEffect / useLayoutEffect callbacks. Use when creating or refactoring components or pages, organizing frontend files, or fetching data from the server.
---
...
12. **Named effects** — `useEffect` / `useLayoutEffect` callbacks must be named functions that describe the effect's purpose, not `() => {}`.
...
## Named `useEffect` / `useLayoutEffect` callbacks

Never pass an anonymous arrow (`() => {}`) to `useEffect` or `useLayoutEffect`. Use a named function whose name states **why** the effect exists. The name shows up in React DevTools and stack traces, and it documents intent at the call site.

```tsx
// ❌ Wrong — anonymous callback
useEffect(() => {
  editor.setEditable(editable)
}, [editor, editable])

// ✅ Correct — name describes the purpose
...
```tsx
// Cleanup-only effects still get a name for the whole effect
useEffect(function clearSkipObserverTimerOnUnmount() {
  return () => window.clearTimeout(skipObserverTimerRef.current)
}, [])
```

### Naming

- Verb phrase: `syncEditable`, `subscribeToSessionStream`, `observeVisibleFiles`, `tickElapsedTimeWhileRunning`.
- Describe the side effect, not the hook (`runEffect` / `handleEffect` are not useful).
- Prefer `function name() { … }` inline at the hook call — do not hoist a separate `const name = () => { … }` just to pass it in.

### Rules

- Applies to `useEffect`, `useLayoutEffect`, and `React.useEffect` / `React.useLayoutEffect`.
- Still never use effects for data fetching (see Data fetching below).
- Event handlers and other callbacks inside the effect body may stay anonymous when they are short and local.
...
| Query keys + helpers | `src/queries/<resource>.ts` | keys, `ensureQueryData` helpers — not React hooks |
| Shared React hooks | `src/hooks/` | UI/behavior hooks that call `use*` |
| Route loader | route file | prefetch via `loader` |
| Component | page or component | `Route.useLoaderData()`, `useMutation` |

Query modules export keys, types, `queryFn`s, and `ensureQueryData` helpers. They do **not** use `useQuery` / `useMutation`. Put those in the component (or a real hook in `src/hooks/` if shared).

### Pattern: read data

**1. Query module** (`src/queries/notifications.ts`):
...
Keep mutation logic in the component (or a colocated hook) that owns the UI — not in `src/queries/`. Promote a shared mutation to `src/hooks/` only when the same `useMutation` is reused across pages.
...
- Export query keys from `src/queries/` so mutations can invalidate consistently.
- Prefer route `loader` + `ensureQueryData` for page-level reads.
- Colocate one-off fetch helpers with the page in `-components/` or the route file; promote to `src/queries/` when reused.
...
- [ ] Query keys centralized in `src/queries/` if fetch is shared
...
- [ ] `useEffect` / `useLayoutEffect`? → named function callback (`function syncFoo() { … }`), not `() => {}`
